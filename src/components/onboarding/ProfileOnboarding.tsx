  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <button
        onClick={onSubmit}
        disabled={saving}
        className="h-[52px] w-full bg-brand-action text-white rounded-[10px] text-[16px] font-semibold hover:opacity-90 transition-opacity disabled:opacity-60"
      >
...
export default function ProfileOnboarding({ redirectUrl }: ProfileOnboardingProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const profilePayload = {
        ...
        'profile.dob': profileData.dobDay && profileData.dobMonth && profileData.dobYear
          ? `${profileData.dobYear}-${profileData.dobMonth.padStart(2, '0')}-${profileData.dobDay.padStart(2, '0')}`
          : '',
        'photo.uploaded': !!profileData.photoUrl,
        'identity.docType': profileData.docType,
        'identity.frontUploaded': profileData.frontUploaded,
        'identity.backUploaded': profileData.backUploaded,
        updatedAt: serverTimestamp(),
      }
      if (snap.exists()) {
        await updateDoc(ref, profilePayload)
      } else {
        await setDoc(ref, {
          uid,
          ...
          photo: { uploaded: !!profileData.photoUrl, url: '' },
          identity: {
            docType: profileData.docType,
            frontUploaded: profileData.frontUploaded,
            backUploaded: profileData.backUploaded,
            skipped: !profileData.frontUploaded,
          },
          createdAt: serverTimestamp(),
        })
      }
      setCompletedSteps([0, 1, 2, 3])
      const dest = redirectUrl ?? '/become-provider/list-service'
      router.push(dest)
    } catch (err) {
      console.error('ProfileOnboarding save error:', err)
      setSaving(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="px-4 pb-12 max-w-lg mx-auto">
        ...
        {currentStep === 3 && (
          <StepReview data={profileData} onEdit={goTo} onSubmit={() => void handleSubmit()} saving={saving} />
        )}
      </div>
    </div>
  )
