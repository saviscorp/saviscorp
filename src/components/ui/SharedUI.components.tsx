    ...
    >
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        >
          ...
          <path d="M2 9h16" />
          <circle cx="14" cy="13.5" r="1.5" fill="currentColor" stroke="none" />
        </svg>
      </div>

      {/* Amount — break-all so it never clips on narrow screens */}
      <p className="text-[36px] font-bold leading-none mb-4 w-full break-all">
        KES {formatted}
      </p>
      ...
      <hr
        className="mb-4"
        style={{ border: 'none', borderTop: '1px solid rgba(255,255,255,0.2)' }}
      />
      ...
      {!empty && (
        <>
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-[13px] font-semibold text-white">{nextPayoutDate}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-[13px]" style={{ color: 'rgba(255,255,255,0.6)' }}>
              Completed this cycle
            </span>
            <span className="text-[13px] font-semibold text-white">{cycleCount} services</span>
          </div>
        </>
      )}
      ...
      {!empty && (
        <button
          ...
          className="mt-4 w-full h-10 rounded-lg text-white font-semibold text-[14px] transition-colors"
          style={{
...
export function BookingCard({
  ...
  const leftBorder = status === 'pending' ? 'border-l-warning' : 'border-l-brand-primary';

  return (
    <div className={`bg-white border border-border-savis rounded-xl p-4 shadow-sm border-l-[3px] ${leftBorder}`}>
      {/* Top row */}
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-brand-light flex items-center justify-center text-[12px] font-bold text-brand-primary">
            {customerInitials}
          </div>
      ...
      <p className="text-[14px] font-medium text-gray-700 mb-1">{serviceName}</p>

      <div className="flex items-center gap-1.5 text-[13px] text-gray-500 mb-0.5">
        <CalendarIcon className="w-3.5 h-3.5 text-brand-primary/50" />
        {dateTime}
      </div>

      {location && (
        <div className="flex items-center gap-1.5 text-[13px] text-gray-500">
          <MapPinIcon className="w-3.5 h-3.5 text-brand-primary/50" />
          {location}
        </div>
      )}
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="mt-3 bg-brand-light border-l-2 border-l-brand-primary/30 rounded-lg p-3 space-y-1">
          {specialInstructions && (
      ...
      {variant === 'manage' && (
        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border-savis">
          {status === 'pending' && (
            <>
              <button
                onClick={() => onAccept?.(id)}
          ...
          {onToggleExpand && (
            <button
              ...
              className="ml-auto bg-brand-light text-brand-primary text-[12px] px-3 py-1 rounded-full hover:bg-brand-primary hover:text-white transition-colors"
            >
